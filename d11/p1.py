...
def turn_clockwise(orientation: Orientation) -> Orientation:
    return {
...
def turn_anti_clockwise(orientation: Orientation) -> Orientation:
    return {
...
class PaintRobot:
    ...
    painted: dict[tuple[int, int], Literal[0, 1]] = field(
    ...
    def turn(self, direction: Literal[0, 1]):
        if direction == 0:
            self.orientation = turn_anti_clockwise(self.orientation)
        elif direction == 1:
            self.orientation = turn_clockwise(self.orientation)

    def paint(self, colour: Literal[0, 1]):
        self.painted[self.coord] = colour

    def detect(self):
        return self.painted.get(self.coord, 0)

    async def run(self):
        count = 0
        while not (self.processer.halted and self.processer.outputs.empty()):
            count += 1
            await self.processer.inputs.put(self.detect())
            self.paint(await self.processer.outputs.get())
            self.turn(await self.processer.outputs.get())
            self.move()


def get_display_size(coords: set[tuple[int, int]]) -> tuple[int, int]:
    return max(x for x, _ in coords) + 1, max(y for _, y in coords) + 1


def display(coords: set[tuple[int, int]]) -> None:
    max_x, max_y = get_display_size(coords)
    for y in range(max_y):
        print("".join("#" if (x, y) in coords else " " for x in range(max_x)))


async def main():
    p = Processor(read_code(), asyncio.Queue(), asyncio.Queue())
    robot = PaintRobot(p)
    await asyncio.gather(p.run(), robot.run())
    display(set(c for c, p in robot.painted.items() if p == 1))
